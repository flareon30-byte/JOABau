...
const GnkInstallationForm = () => {
    ...
    const getGPSLocation = () => {
        ...
        if (!navigator.geolocation) {
            setErrorMessage(t('activation_form.geolocation_unsupported'));
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                try {
                    ...
                    setErrorMessage(t('activation_form.reverse_geocode_warn'));
                }
                setLoadingLocation(false);
            },
            (error) => {
                console.error(error);
                setErrorMessage(t('activation_form.gps_error'));
                setLoadingLocation(false);
            },
    ...
    const handleSubmit = async () => {
        if (!address.street) {
            setErrorMessage(t('activation_form.street_required'));
            return;
        }
        if (sigPad.current && sigPad.current.isEmpty()) {
            setErrorMessage(t('activation_form.signature_required'));
            return;
        }
        ...
        try {
            ...
            setSuccessMessage(t('activation_form.success_submit'));
            ...
            setErrorMessage(serverMsg ? `Error: ${serverMsg}` : t('activation_form.error_submit'));
    ...
    return (
        <div className="max-w-3xl mx-auto pb-24">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">{t('activation_form.title')}</h1>
                    <p className="text-sm text-slate-500 font-medium">{user.activeClientCompany?.name || t('activation_form.loading_client')}</p>
                </div>
            </div>
                ...
                {step === 1 && (
                    <div className="space-y-6 animate-fadeIn">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            >
                                <Navigation size={16} />
                                {loadingLocation ? t('activation_form.searching') : t('activation_form.auto_locate')}
                            </button>
                        </div>
                        ...
                        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 space-y-4">
                            <div className="grid grid-cols-3 gap-4">
                                <div className="col-span-2">
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('activation_form.street')}</label>
                                    <input 
                                        type="text" 
                                        value={address.street}
                                        onChange={(e) => setAddress({...address, street: e.target.value})}
                                        placeholder={t('activation_form.street_placeholder')}
                                        className="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-joa-blue outline-none" 
                                    />
                                </div>
                                <div className="col-span-1">
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('activation_form.number')}</label>
                                    <input 
                                        type="text" 
                                        value={address.number}
                                        onChange={(e) => setAddress({...address, number: e.target.value})}
                                        placeholder={t('activation_form.number_placeholder')}
                                        className="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl focus:ring-2 focus:ring-joa-blue outline-none" 
                                    />
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">{t('activation_form.city')}</label>
                                <input 
                                    type="text" 
                                    value={address.city}
                        ...
                        {availableItems.length > 0 && (
                            <div className="space-y-4 pt-2">
                                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                                    <Tag size={14} className="text-joa-blue" /> {t('activation_form.installed_units')} ({user.activeClientCompany?.name})
                                </label>
                        ...
                        <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 mt-6 space-y-4">
                            <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2 mb-2">
                                <Shield className="text-joa-blue" size={16} /> {t('activation_form.client_network_data')}
                            </h3>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('activation_form.first_name')}</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('activation_form.last_name')}</label>
                                    <input 
                                        type="text" 
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('activation_form.olt')}</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('activation_form.pon')}</label>
                                    <input 
                                        type="text" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">{t('activation_form.port')}</label>
                                    <input 
                                        type="number" 
                            ...
                            <div>
                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1">
                                    <QrCode size={12} /> {t('activation_form.gpon_serial')}
                                </label>
                                <input 
                                    type="text" 
                        ...
                        <div className="pt-2 flex items-center gap-3">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                />
                                <span className="text-sm font-bold text-slate-700">{t('activation_form.connection_ready')}</span>
                            </label>
                        </div>

                        <div className="pt-2">
                            <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-2">
                                <MessageSquare size={14} /> {t('activation_form.comments')}
                            </label>
                            <textarea 
                                value={comments}
                                onChange={(e) => setComments(e.target.value)}
                                placeholder={t('activation_form.comments_placeholder')}
                        ...
                        <div className="pt-4 text-right">
                            <button 
                                onClick={() => {
                                    if(!address.street) setErrorMessage(t('activation_form.street_required'));
                                    else setStep(2);
                                }}
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-fadeIn">
                        ...
                        <div className="bg-blue-50 border border-blue-100 p-4 rounded-xl text-sm text-blue-800 flex gap-3 items-start mb-6 font-medium">
                            ...
                            <p>
                                {t('activation_form.photo_instructions')}
                            </p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
                            ...
                            <div className="space-y-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t('activation_form.photo_huep')}</label>
                                    ...
                                    {specialPhotos.photoHuep ? (
                                        ...
                                        <div className="flex flex-col gap-2 w-full px-4 text-center">
                                            <button onClick={() => huepInputRef.current.click()} className="flex flex-col items-center justify-center py-2 bg-blue-50 text-joa-blue rounded-xl hover:bg-blue-100 transition-colors border border-blue-100">
                                                <CameraIcon size={24} />
                                                <span className="text-[9px] font-black mt-1 uppercase">{t('activation_form.camera')}</span>
                                            </button>
                                            <button onClick={() => {
                                                ...
                                                <ImageIcon size={20} />
                                                <span className="text-[9px] font-black mt-1 uppercase">{t('activation_form.gallery')}</span>
                                            </button>
                                        </div>
                                    )}
                            ...
                            {/* PHOTO MODEM */}
                            <div className="space-y-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t('activation_form.photo_modem')}</label>
                                <div className="relative aspect-square rounded-2xl overflow-hidden border-2 border-slate-200 bg-slate-50 flex items-center justify-center group shadow-sm transition-all hover:border-joa-blue/40">
                            ...
                            {/* PHOTO OTDR */}
                            <div className="space-y-2">
                                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">{t('activation_form.photo_otdr')}</label>
                                <div className="relative aspect-square rounded-2xl overflow-hidden border-2 border-slate-200 bg-slate-50 flex items-center justify-center group shadow-sm transition-all hover:border-joa-blue/40">
                        ...
                        <div className="border-t border-slate-100 pt-6">
                            <h3 className="text-sm font-bold text-slate-600 mb-4 flex items-center gap-2">
                                <ImageIcon size={18} className="text-slate-400" /> {t('activation_form.other_photos')}
                            </h3>
                            <div className="grid grid-cols-3 sm:grid-cols-4 gap-4 mb-4">
                                ...
                                <div className="border-2 border-dashed border-blue-400 bg-blue-50/50 rounded-2xl aspect-square flex flex-col items-center justify-center p-4 hover:bg-blue-100 transition-colors cursor-pointer relative shadow-sm hover:shadow-md transition-all">
                                    ...
                                    <div className="text-[9px] text-blue-700 font-extrabold uppercase text-center leading-tight whitespace-pre-line">
                                        {t('activation_form.take_photo')}
                                    </div>
                                </div>
                                ...
                                <div className="border-2 border-dashed border-slate-300 bg-white rounded-2xl aspect-square flex flex-col items-center justify-center p-4 hover:bg-slate-50 transition-colors cursor-pointer relative shadow-sm hover:shadow-md transition-all">
                                    ...
                                    <div className="text-[9px] text-slate-600 font-extrabold uppercase text-center leading-tight whitespace-pre-line">
                                        {t('activation_form.gallery_multi')}
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                {step === 3 && (
                    <div className="space-y-6 animate-fadeIn">
                        <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2 mb-6">
                            ...
                            {t('activation_form.step3')}
                        </h2>

                        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('activation_form.client')}</h4>
                                    <p className="text-sm font-bold text-slate-800 truncate">
                                        {muenetData.customerFirstName} {muenetData.customerLastName}
                                    </p>
                                </div>
                                <div>
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('activation_form.address')}</h4>
                                    <p className="text-sm font-bold text-slate-800 truncate">
                            ...
                            <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-200">
                                <div>
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('activation_form.olt')}</h4>
                                    <p className="text-sm font-semibold text-slate-700">{muenetData.olt || '-'}</p>
                                </div>
                                <div>
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('activation_form.pon')}</h4>
                                    <p className="text-sm font-semibold text-slate-700">{muenetData.pon || '-'}</p>
                                </div>
                                <div>
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('activation_form.port')}</h4>
                            ...
                            <div className="pt-3 border-t border-slate-200">
                                ...
                                <p className="text-sm font-mono font-bold text-slate-700">{muenetData.gponSerialNumber || t('activation_form.not_defined')}</p>
                            </div>

                            {/* Review Items */}
                            {Object.entries(selectedQuantities).some(([_, qty]) => qty > 0) && (
                                <div className="pt-3 border-t border-slate-200">
                                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">{t('activation_form.installed_units')}</h4>
                            ...
                            <div className="pt-4 border-t border-slate-200">
                                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                    <PenTool size={12} /> {t('activation_form.client_signature')}
                                </label>
                            ...
                            >
                                {isSubmitting ? (
                                    <>
                                        ...
                                        {t('activation_form.sending')}
                                    </>
                                ) : (
                                    <>{t('activation_form.send_report')}</>
                                )}
                            </button>
                        </div>
                    </div>
